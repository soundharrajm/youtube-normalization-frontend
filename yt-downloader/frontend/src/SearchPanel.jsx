...
// Duration maps to YouTube API videoDuration param
const FILTERS = [
  { label:'All',    value:'' },
  { label:'Short',  value:'short' },   // < 4 min
  { label:'Medium', value:'medium' },  // 4–20 min
  { label:'Long',   value:'long' },    // > 20 min
]

// Content type maps to YouTube API videoType or title keyword
const CONTENT_TYPES = [
  { label:'All Videos', value:'',        ytType:'video' },
  { label:'Shorts',     value:'short',   ytType:'video' },   // ≤60s — use duration=short
...
export default function SearchPanel({ onAddUrl, onClose }) {
  ...
  // Build proper YouTube API params — no more appending to query string
                ...
                return (
                  <div key={video.video_id} style={{ background:'rgba(255,255,255,0.03)', border:isAdded?'1px solid rgba(16,185,129,0.3)':'1px solid rgba(255,255,255,0.08)', borderRadius:12, overflow:'hidden', display:'flex', flexDirection:'column', transition:'border-color 0.2s' }}>
                    ...
                    <div style={{ padding:'0 12px 12px' }}>
                      <button onClick={()=>!isAdded&&handleAdd(video)} style={{ width:'100%', padding:'7px', borderRadius:8, fontSize:12, fontWeight:700, fontFamily:'inherit', cursor:isAdded?'default':'pointer', border:isAdded?'1px solid rgba(16,185,129,0.3)':'1px solid rgba(139,92,246,0.3)', background:isAdded?'rgba(16,185,129,0.12)':'rgba(139,92,246,0.12)', color:isAdded?'#34d399':'#a78bfa' }}>
